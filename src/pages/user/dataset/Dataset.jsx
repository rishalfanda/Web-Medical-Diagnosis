        ...
        <div className={`${role === "admin" ? "bg-white/80 backdrop-blur-sm rounded-2xl border border-white/20 shadow-xl overflow-hidden" : "bg-gray-800/30 backdrop-blur-sm rounded-2xl border border-gray-700/50 shadow-xl overflow-hidden"}`}>
          ...
          <div className={`${role === "admin" ? "p-6 border-b border-gray-200/50" : "p-6 border-b border-gray-700/50"}`}>
            <div className="flex justify-between items-center">
              <div>
                <h2 className={`${role === "admin" ? "text-xl font-bold text-gray-800" : "text-xl font-bold text-white"}`}>Dataset Records</h2>
                <p className={`${role === "admin" ? "text-gray-600 text-sm mt-1" : "text-gray-400 text-sm mt-1"}`}>AI-powered medical dataset results</p>
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className={`${role === 'admin' ? "bg-gray-50/50" : "bg-gray-700/30"}`}>
                <tr>
                  <th className={`${role === 'admin' ? "text-gray-600" : "text-gray-300"} px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider`}>
                    Nama Dataset
                  </th>
                  <th className={`${role === 'admin' ? "text-gray-600" : "text-gray-300"} px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider`}>
                    Lokasi
                ...
                {dataset.map((record, index) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      <p className={`${role === "admin" ? "text-gray-900" : "text-gray-300"} text-sm`}>{record.nama_dataset}</p>
                    </td>

                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-2">
                        <span className={`${role === "admin" ? "text-gray-900" : "text-gray-300"} text-sm`}>{record.lokasi}</span>
                      </div>
                    </td>
