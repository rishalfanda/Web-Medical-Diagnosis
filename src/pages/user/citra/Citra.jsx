...
function Citra() {
  ...
  <div className={`${isAdminOrNull ? "p-6 border-b border-gray-200/50" : "p-6 border-b border-gray-700/50"}`}>
    <div className="flex justify-between items-center">
      <div>
        {citra.length > 0 && (
          <h2 className={`${isAdminOrNull ? "text-xl font-bold text-gray-800" : "text-xl font-bold text-white"}`}>
            Citra record at {citra[0].dataset?.nama_dataset}
          </h2>
        )}
        <p className={`${isAdminOrNull ? "text-gray-600 text-sm mt-1" : "text-gray-400 text-sm mt-1"}`}>AI-powered medical citra results</p>
      </div>
  ...
  <div className="overflow-x-auto">
    <table className="w-full">
      <thead className={`${isAdminOrNull ? "bg-gray-50/50" : "bg-gray-700/30"}`}>
        <tr>
          <th className={`${isAdminOrNull ? "text-gray-600" : "text-gray-300"} px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider`}>Image</th>
          <th className={`${isAdminOrNull ? "text-gray-600" : "text-gray-300"} px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider`}>Kode Citra</th>
          <th className={`${isAdminOrNull ? "text-gray-600" : "text-gray-300"} px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider`}>Diagnosis</th>
          <th className={`${isAdminOrNull ? "text-gray-600" : "text-gray-300"} px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider`}>Upload Date</th>
          <th className={`${isAdminOrNull ? "text-gray-600" : "text-gray-300"} px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider`}>Actions</th>
        </tr>
      </thead>
      <tbody className={`divide-y ${isAdminOrNull ? "divide-gray-200/50" : "divide-gray-700/50"}`}>
        {citra.map((record, index) => (
          ...
          >
            ...
            <td className="px-6 py-4">
              <span className={`${isAdminOrNull ? "text-gray-900" : "text-gray-300"} text-sm`}>{record.kode_citra}</span>
            </td>
            <td className="px-6 py-4">
              <span className={`${isAdminOrNull ? "text-gray-900" : "text-gray-300"} text-sm`}>{record.diagnosis}</span>
            </td>
            <td className="px-6 py-4">
