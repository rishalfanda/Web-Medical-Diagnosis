...
function Citra() {
    ...
    if (role != "superadmin") {
      if (citra[0].dataset?.instance_id != instance_id) {
        return (
          ...
          <tbody className={`divide-y ${isAdminOrSuperadmin ? "divide-gray-200/50" : "divide-gray-700/50"}`}>
            {citra.map((record, index) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  <span className={`${isAdminOrSuperadmin ? "text-gray-900" : "text-gray-300"} text-sm`}>{record.kode_citra}</span>
                </td>
                <td className="px-6 py-4">
                  <span className={`${isAdminOrSuperadmin ? "text-gray-900" : "text-gray-300"} text-sm`}>{record.diagnosis}</span>
                </td>
                <td className="px-6 py-4">
